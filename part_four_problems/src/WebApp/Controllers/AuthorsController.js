...
const { BookResponseParam } = require('../Models/Book.js');
...
Router.get('/:id/books', async (req, res) => {
    ...
    const books = await AuthorsRepository.getByAuthorAsync(id);
    
    if (!books) {
        throw new ServerError(`Author with id ${id} does not exist!`, 404);
    }
    console.log(books);
    ResponseFilter.setResponseDetails(res, 200, books.map(book => new BookResponseParam(book)));
});
